...
- [`createtzfromwin.dpr`](tools/createtzfromwin.dpr) generates a 10 KB `tz.dat` from an up-to-date Windows 11 system, to have detailed time zones information for `TSynTimeZone` as `{$R ..\src\mormot.tz.res}` on Linux/POSIX systems, without the complexity of the POSIX time packages.
