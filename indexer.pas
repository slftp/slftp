...
function IndexerAlaive:boolean;
begin
if indexes = nil then result:= False else result:=True;
end;    


procedure indexerBeginTransaction();
begin
  if indexes = nil then exit;
...
begin
  if indexes = nil then exit;
  if indexerInsert = nil then exit;
...
begin
  if indexes = nil then exit;
  if indexerDelete = nil then exit;
...
begin
  Result:= '';
  if indexes = nil then exit;
  if indexerSelect = nil then exit;
...
begin
  ...
  try
    ...
    while indexes.Step(indexerSelectPartially) do
      Result:= Result+ indexes.column_text(indexerSelectPartially,0)+'-'+indexes.column_text(indexerSelectPartially,1)+'='+indexes.column_text(indexerSelectPartially,2)+' => '+ indexes.column_text(indexerSelectPartially,3) + #13#10;
  finally
    indexer_lock.Leave;
  end;
end;
