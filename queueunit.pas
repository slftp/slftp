...
type
  ...
  fBusyDestinations: TDictionary<TObject, integer>;
    fQueueDirty: Boolean; //< @true when the queue needs to be re-sorted before the next assignment
...
begin
  ...
  // sort happens in Execute once a race task can be assigned, collapsing
  // repeated sorts (e.g. after every dirlist of a race) into a single one
  try
