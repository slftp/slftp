...
begin
  ...
  try
    ...
    begin
      ...
      ps1.MkdirError(dir);
      readyerror := True;
      Debug(dpSpam, c_section, '<-- %s (failure after %d ms)', [tname, MilliSecondsBetween(Now, fMkdirStart)]);
      Result := True;
      exit;
    end;
  ...
  Debug(dpSpam, c_section, '<-- %s (success after %d ms)', [tname, MilliSecondsBetween(Now, fMkdirStart)]);
