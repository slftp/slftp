...
function IrcAffilRoutes(const netname, channel, params: String): boolean;
...
function _IrcSetAffilRoute(const netname, channel, params: String; lock: boolean = False): boolean;
var
  source, dest, admin_site: String;
  rcmd: TCommandLineReader;
  c1, c2, sw1, sw2: String;
...
begin
  ...
  // Parse the params - same as _IrcSetRoute but for affil routes
  rcmd := TCommandLineReader.create();
  try
    try
      rcmd.allowDOSStyle := True;
      ...
      begin
        irc_addtext(Netname, Channel, '<c4><b>%s</b></c>', [e.Message]);
        Debug(dpError, section, '[EXCEPTION] IrcAffilRoutes(rcmd.parse): %s', [e.Message]);
        exit;
      end;
    end;
  ...
  // Same validations as _IrcSetRoute
  if (source = admin_site) or (dest = admin_site) then
  begin
    irc_addtext(Netname, Channel, '<c4><b>You can not use admin site with this function</b>.</c>');
    exit;
  ...
  site := nil;
  try
    // Site lookup logic - same as _IrcSetRoute
    if source = '*' then
    begin
      for i := 0 to sites.Count - 1 do
      begin
        ...
        if site.Name = admin_site then Continue;
        if (c1 <> '') and (AnsiLowerCase(site.Country) <> ('.' + AnsiLowerCase(c1))) then Continue;
        if (sw1 <> '') and (StringToSiteSoftWare(sw1) <> site.sw) then Continue;
        source_sites.Add(site.Name);
      end;
    end
    else
    ...
    begin
      ...
      begin
        ...
        if site = nil then Continue;
        if site.Name = admin_site then Continue;
        if (c2 <> '') and (AnsiLowerCase(site.Country) <> ('.' + AnsiLowerCase(c2))) then Continue;
        if (sw2 <> '') and (StringToSiteSoftWare(sw2) <> site.sw) then Continue;
        dest_sites.Add(site.Name);
      end;
    end
    else
    ...
    // Apply affil routes using affilspeed-from-* instead of speed-from-*
    for i := 0 to source_sites.Count - 1 do
    begin
      for j := 0 to dest_sites.Count - 1 do
      begin
        if (source_sites[i] = dest_sites[j]) then Continue;

        backtext := '';
        if back then backtext := ' (and backroute)';
        
        if speed > 0 then
        begin
          irc_addtext(Netname, Channel, 'Affil route from <b>%s</b> to <b>%s</b> set to %d%s', [source_sites[i], dest_sites[j], speed, backtext]);
        end
        else
        begin
          irc_addtext(Netname, Channel, 'Affil route from <b>%s</b> to <b>%s</b> removed%s', [source_sites[i], dest_sites[j], backtext]);
        end;
        ...
        begin
          if speed > 0 then
          begin
            // Use affilspeed-from-* for affil routes (separate from normal routes)
            sitesdat.WriteInteger('affilspeed-from-' + source_sites[i], dest_sites[j], speed);
            sitesdat.WriteInteger('affilspeed-to-' + dest_sites[j], source_sites[i], speed);
            if back then
            begin
              sitesdat.WriteInteger('affilspeed-from-' + dest_sites[j], source_sites[i], speed);
              sitesdat.WriteInteger('affilspeed-to-' + source_sites[i], dest_sites[j], speed);
            end;
          end
          else
          begin
            sitesdat.DeleteKey('affilspeed-from-' + source_sites[i], dest_sites[j]);
            sitesdat.DeleteKey('affilspeed-to-' + dest_sites[j], source_sites[i]);
            if back then
            begin
              sitesdat.DeleteKey('affilspeed-from-' + dest_sites[j], source_sites[i]);
              sitesdat.DeleteKey('affilspeed-to-' + source_sites[i], dest_sites[j]);
            end;
          end;
        end;
      end;
    ...
    if not DoIt then
      irc_addtext(Netname, Channel, 'Affil routes were not really added. Check if you are satisfied and add --apply to the command.');
...
var
  x, affilx: TStringList;
  ii, affil_ii, i: integer;
  ss, affilss, fIdentifier, fArrowDirection: String;
begin
  ...
  try
    ...
    // Read affil routes
    sitesdat.ReadSection('affilspeed-' + fIdentifier +'-' + sitename, affilx);
    affilss := '';
    affil_ii := affilx.Count;
    for i := 0 to affilx.Count - 1 do
    begin
      if affilss <> '' then
        affilss := affilss + ', ';
      affilss := affilss + '"' + affilx[i] + ' ' + sitesdat.ReadString('affilspeed-' + fIdentifier + '-' + sitename, affilx[i], '') + '(A)' + '"';
    end;
  ...
  // Display combined routes
  if (ss <> '') or (affilss <> '') then
  begin
    if (ss <> '') and (affilss <> '') then
    begin
      // Both normal and affil routes exist
      IrcLineBreak(Netname, Channel, ss + ', ' + affilss, AnsiChar('"'), Format('<b>%s (%d+%dA)</b> %s ', [sitename, ii, affil_ii, fArrowDirection]));
    end
    else if ss <> '' then
    begin
      // Only normal routes
    ...
    begin
      // Only affil routes
      IrcLineBreak(Netname, Channel, affilss, AnsiChar('"'), Format('<b>%s (%dA)</b> %s ', [sitename, affil_ii, fArrowDirection]));
    end;
  end;
end;
...
begin
  ...
  if (_IrcSetAffilRoute(Netname, Channel, params, False)) then
    Result := True;
end;
