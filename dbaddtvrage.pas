...
procedure dbaddtvrage_SaveTVRage(tv_showid: string; tvrage: TDbTVRage;
  rls: string = '');
...
uses DateUtils, SysUtils, Math, configunit, mystrings, irccommandsunit,
  ...
  tasktvragelookup, pazo, mrdohutils;
...
begin
  try
    addtvrageDB.ExecSQL(sql_addtvrage, [rls_showname, tv_showid,
      tv_showname, tv_showurl, IntToStr(tv_premiered_year), tv_country,
      tv_status, tv_classification, tv_genres.DelimitedText, tv_network,
      IntToStr(tv_runtime), BoolToStr(tv_running), IntToStr(tv_endedyear)]);
    //, IntToStr(tv_seasons)]);
  except
    on e: Exception do
    begin
...
begin
  try
    ...
    on e: Exception do
    begin
      Debug(dpError, section, Format('[EXCEPTION] TDbTVRage.PostResults: %s ',
        [e.Message]));
...
begin
  ...
  if tv_classification = 'Scripted' then
    tr.scripted := True;
...
begin
  ...
  try
    ...
    begin
      Result := False;
      Debug(dpError, section, Format('[EXCEPTION] dbaddtvrage_update_show: %s ',
        [e.Message]));
    end;
  end;
  ...
  begin
    try
      ...
      begin
        ...
        tvrage.tv_running :=
          StrToBoolDef(addtvrageDB.column_text(gettvrage, 11), False);
...
begin
  ...
  begin
    try
      gettvrage := addtvrageDB.Open('SELECT * FROM addtvrage WHERE tv_showid = "' +
        tv_showid + '"');
      if addtvrageDB.Step(gettvrage) then
      begin
    ...
    begin
      try
        if spamcfg.ReadBool('addinfo', 'tvrageupdate', True) then
          irc_Addadmin(Format('<c3>[TVRAGE]</c> %s %s now has TVRage infos (%s)',
...
begin
  ...
  try
    ...
    on e: Exception do
    begin
      Debug(dpError, section, Format('[EXCEPTION] dbaddtvrage_GetCount: %s ',
        [e.Message]));
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    // addtvrage DB
    addtvrageDB := TslSqliteDB.Create(db_pre_name,
    ...
    sql_addtvrage   := addtvrageDB.Open(
      'INSERT OR IGNORE INTO addtvrage (rls_showname, tv_showid, tv_showname, tv_showurl, tv_premiered_year, tv_country, tv_status, tv_classification, tv_genres, tv_network, tv_runtime, tv_running, tv_endedyear)' +//, tv_seasons ,last_updated)' +
      'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');//, ?, ?)');
