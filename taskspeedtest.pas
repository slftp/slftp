...
begin
  try
    Result := Format('DELSPEEDTESTFILE <b>%s</b> -> %s', [site1, speedtestfilename]);
  except
...
ujra:
  ...
  // here you have to read the lambs down the drum
  if not s.Send('REST 0') then goto ujra;
  if not s.Read() then goto ujra;

  if not s.Send('STOR ' + speedtestfilename) then goto ujra;
  ...
  begin
    ...
    begin
      irc_addtext(s.todotask, 'Couldnt connect to site (%s)', [idTCP.error]);
      s.DestroySocket(False);
      readyerror := True;
      exit;
    end;
  ...
  begin
    ...
    begin
      irc_addtext(s.todotask, 'Couldnt negotiate SSL connection (%s)', [idTCP.error]);
      s.DestroySocket(False);
      readyerror := True;
      exit;
    end;
  ...
  begin
    ...
    begin
      irc_addtext(s.todotask, 'Couldnt send to site (%s)', [s.error]);
      s.DestroySocket(False);
      readyerror := True;
      exit;
    end;
...
begin
  try
    Result := Format('UPLOADSPEEDTESTFILE <b>%s</b> -> %s', [site1, speedtestfilename]);
  except
