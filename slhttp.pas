...
type
  ...
  public
    ...
    function Get(const url: string; params: TStringList; output: TStream = nil): Boolean; overload;
...
function TslHTTP.Get(const url: string; params: TStringList; output: TStream = nil): Boolean;
const headernekszantresz = 8192;
var s, ss, paramsstr: string;
    uri: string;
    i: Integer;
    olvass: Integer;
    httpsmode: Boolean;
...
begin
  ...
  begin
    ...
    begin
      if i <> 0 then paramsstr:= paramsstr + '&';
      paramsstr:= paramsstr + UrlEncode(params.Names[i]) + '='+UrlEncode(params.ValueFromIndex[i]);
    end;
  end;
...
Port:= 80;
  ...
  if not Connect(Timeout) then exit;

  if httpsmode then
    if not TurnToSSL(Timeout) then exit;

  if not WriteLn('GET '+uri+' HTTP/1.0') then exit;
  if not WriteLn('Host: '+host) then exit;
  for i:= 0 to CustomHeaders.Count -1 do
    if not WriteLn(CustomHeaders.Names[i]+': '+CustomHeaders.ValueFromIndex[i]) then exit;

  if not WriteLn('') then exit;

  if output = nil then olvass:= 0 else olvass:= headernekszantresz;
  if not Read(Response, Timeout, olvass, True) then exit;

  Response.Position:= 0;
  s:= Response.ReadString(16384);

  ResponseStartsAt:= Pos(#13#10#13#10, s);
  if ResponseStartsAt = 0 then exit;
  inc(ResponseStartsAt, 4);

  ResponseCode:= StrToIntDef(Copy(s, 10, 3),0);
  if ResponseCode = 0 then exit;
  ...
  begin
    ...
    i:= Pos(': ', ss);
    if i <> 0 then
      ResponseHeaders.Add(Copy(ss, 1, i-1)+'='+ Copy(ss, i+2, Length(ss)));
  end;
  if ResponseHeaders.Count = 0 then exit;
  ...
  begin
    ...
    begin
      ...
      if not Read(output, Timeout, 0, True) then exit;
    end;
...
Port:= 80;
  ...
  if not WriteLn('POST '+uri+' HTTP/1.0') then exit;
  if not WriteLn('Host: '+host) then exit;
  if not WriteLn('Content-Length: '+IntToStr(Length(content))) then exit;
  for i:= 0 to CustomHeaders.Count -1 do
    if not WriteLn(CustomHeaders.Names[i]+': '+CustomHeaders.ValueFromIndex[i]) then exit;
  if not WriteLn('') then exit;

  if not WriteLn(content) then exit;
  if not WriteLn('') then exit;

  if not Read(Response, Timeout, 0, True) then exit;
...
begin
  ...
  contentstr:= '';
    ...
    begin
      if i <> 0 then contentstr:= contentstr + '&';
      contentstr:= contentstr + UrlEncode(content.Names[i]) + '='+UrlEncode(content.ValueFromIndex[i]);
    end;
