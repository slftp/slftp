...
begin
  if not Sorted then
    ...
    if Find(S, Result) then
      case Duplicates of
        dupIgnore: Exit;
        dupError: raise Exception.Create(sDuplicateInt);
      end;
  InsertItem(Result, S);
end;
...
begin
  if (Index < 0) or (Index >= FCount) then raise Exception.Create(SListIndexError);
  Changing;
  Dec(FCount);
...
begin
  if (Index1 < 0) or (Index1 >= FCount) then raise Exception.Create(SListIndexError);
  if (Index2 < 0) or (Index2 >= FCount) then raise Exception.Create(SListIndexError);
  Changing;
...
begin
  if Sorted then raise Exception.CReate(SSortedListError);
  if (Index < 0) or (Index > FCount) then raise Exception.Create(SListIndexError);
  InsertItem(Index, S);
end;
...
function TIntList.Sum: Integer;
var i: Integer;
begin
  Result:= 0;
  for i:= 0 to Count -1 do
    inc (Result, Integers[i]);
end;
