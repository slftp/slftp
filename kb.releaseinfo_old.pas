...
type
  ...
  { update function(s) }
  IUpdate = interface
    function Aktualizald(const extrainfo: String): boolean; abstract;
    // TODO
  end;


  { default class from which all T*Release classes have to inherit from }
  TReleaseAncestor = class(TInterfacedObject)
    // TODO
    function Aktualizald(const extrainfo: String): boolean; abstract;
    procedure SetPretime(TimeStamp: int64 = 0); abstract
    ...
    function AsText(const aPazoID: Integer): String; abstract;

    { Get name of this class
      @returns(Name of this class) }
    class function Name: String; abstract;
    ...
    { Gets a value indicating whether there is a SectionHandler for the given section (e.g. TIMDBRelease, TTVRelease, ...)
      @returns(@true, if there is a section handler for the given section, otherwise @false) }
    class function SectionAccepted(const aSection: String): boolean; abstract;
  end;

  { basic info for each release - mostly parsed from given ReleaseName }
  ...
  public
    //constructor Create(const aRlsname, aSection: String; const aCurrentYear, aYear: Integer);

    //TODO: the initial values can be stored from within Create but the values needs to be updated once something was changed -> update function from Controller?
    // maybe don't save anything from this class but only from Controller classes -> better separation
  published
    ...
    property UTCPreTime: Int64 read FUTCPreTime; // TODO: maybe put into separate class? - could interoperate with addpre db directly
    property PreTimeSource: String read FPreTimeSource; // TODO: maybe put into separate class? - could interoperate with addpre db directly
  ...
  { basic info controller that does the parsing }
  ...
  TBasicController = class(TAggregatedObject, IBasicInfo)
  private
    //FBasicInfo: IBasicInfo;
    FBasicInfoStore: TBasicInfoStore;
// TODO: separate functions to parse year, internal, etc
...
// TODO: this class should probably take care of adding/updating the stored info into the database by TSQLRest
  public
// TODO: needs a special parameter as language parsing is different for MP3 and MVID
  ...
  //TRelease = class(TReleaseAncestor, IBasicInfo)
    // TODO

// TMP3Release -> TAudioRelease -- maybe renaming Mp3 -> Audio as it's for mp3/flac/mvid?
// TIMDbRelease -> TMovieRelease or better TVideoInfo
