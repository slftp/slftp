...
procedure RemovePazoDirTasks(const aPazoID: integer; const aSitename: String);
  begin
    FindSiteByName('', aSitename).RemovePazoDirTasks(aPazoID);
  end;
      ...
      begin
        ...
        try
          ...
          begin
            ...
            // guard against fTodotask being nil'd concurrently by RemovePazo
            try
              if todotask <> nil then
                todotask.readyerror := True;
            except
              // fTodotask was cleared by another thread while Execute was running
            end;
          end;
        end;
        ...
        // Use todotask (captured before Execute) instead of self.todotask.
...
begin
  ...
  // Check if site is on login cooldown
  if site.LoginCooldownActive then
  begin
    Debug(dpSpam, section, '[LOGIN COOLDOWN] Slot %s login blocked: site on login cooldown (%ds remaining)',
      [Name, site.LoginCooldownRemainingSeconds]);
    Exit(False);
  end;
...
begin
  ...
  Result := fConnect_timeout ;
end;

function TSite.GetIdleInterval: integer;
begin
  Result := fIdleInterval;
end;
...
begin
  Result := fKillConnectionOnStalledTransferSeconds;
end;
...
begin
  try
    ...
    begin
      Debug(dpError, section,
        Format('[EXCEPTION] _mySpeedComparer : %s',
        [e.Message]));
      Result := 0;
    end;
  end;
