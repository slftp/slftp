...
function MD5File(const FileName: {$IFDEF UNICODE}RawByteString{$ELSE}String{$ENDIF}): TMD5Digest;
...
{ The MD5DigestToStr function converts the result of
  ...
  hexadecimal digits }
function MD5DigestToStr(const Digest: TMD5Digest): {$IFDEF UNICODE}RawByteString{$ELSE}String{$ENDIF};
