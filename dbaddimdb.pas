...
var
  FIMDbData: TIMDbDataRecord;
begin
  FIMDbData := TIMDbDataRecord.CreateAndFillPrepare(ImdbDatabase.Client, 'IMDbID = ?', [], [aIMDbId]);
  try
...
begin
  ...
  try
    ...
    begin
      ...
      fIMDbReleaseDatesRecord := TIMDbReleaseDatesRecord.CreateAndFillPrepare(ImdbDatabase.Client, 'IMDbData = ?', [], [aImdbRec.ID]);
      ...
      fIMDbBomData := TIMDbBomDataRecord.CreateAndFillPrepare(ImdbDatabase.Client, 'IMDbData = ?', [], [aImdbRec.ID]);
...
begin
  Result := nil;
  fImdbRec := TIMDbDataRecord.CreateAndFillPrepare(ImdbDatabase.Client, 'IMDbID = ?', [], [aIMDbId]);
...
begin
  Result := False;
  fId_FIMDbData := TIMDbDataRecord.CreateAndFillPrepare(ImdbDatabase.Client, 'IMDbID = ?', [], [aIMDbId]);
  try
...
begin
  ...
  try
    ...
    fIMDbAlsoKnownAsRecordRec := TIMDbAlsoKnownAsRecord.CreateAndFillPrepare(ImdbDatabase.Client, 'IMDbData = ? and IMDbTitleCleaned = ? and Country = ?', [], [fIMDbDataRec.ID, fCleanedMovieName, fReleasenameCountry]);
    ...
    begin
      ...
      fIMDbAlsoKnownAsRecordRec.IMDbData := fIMDbDataRec.AsTOrm;
    ...
    begin
      ...
      begin
        ...
        fIMDbReleaseDatesRecordRec.IMDbData := fIMDbDataRec.AsTOrm;
    ...
    begin
      ...
      begin
        ...
        fIMDbBomDataRecordRec.IMDbData := fIMDbDataRec.AsTOrm;
...
begin
  ...
  try
    ...
    begin
      fAlsoKnownDataRec := TIMDbAlsoKnownAsRecord.CreateAndFillPrepareJoined(ImdbDatabase.Client,
      ...
      begin
        fMovieImdbDataRec := TIMDbDataRecord.CreateAndFillPrepare(ImdbDatabase.Client, 'ID = ?', [], [fAlsoKnownDataRec.IMDbData.ID]);
        fMovieImdbDataRec.FIMDbCountries := TStringList.Create;
        fMovieImdbDataRec.FIMDbLanguages := TStringList.Create;
        fMovieImdbDataRec.FIMDbGenres := TStringList.Create;
