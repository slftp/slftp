...
var
  imdb_remove_words_list: TStringList; //< list of regular expressions from slftp.imdbreplace which are used to strip scene tags from release names
  ImdbDatabase: TSQLRestClientDB; //< ORM database connection for persistent IMDB data
  ImdbDbModel: TSQLModel; //< ORM model for the IMDB database
  addimdbcmd: String; //< IRC command to manually add IMDB data for a release (from slftp.ini)
...
var
  last_addimdb: THashedStringList; //< in-memory cache of recently added IMDB data (key: releasename)
  dbaddimdb_cs: TSlCriticalSection2; //< critical section to protect the in-memory caches and regex
...
begin
  ...
  try
    ...
    try
      ...
      begin
        Debug(dpSpam, section, Format('[IMDB-FLOW22] Calling SetIMDBRelease for pazo: %s', [rls]));
        imdbdata.SetIMDBRelease(TIMDBRelease(fPazo.rls));
        Debug(dpSpam, section, Format('[IMDB-FLOW23] SetIMDBRelease completed for pazo: %s', [rls]));
        ...
        begin
          ...
          if (ps <> nil) then
          begin
            Debug(dpSpam, section, Format('[IMDB-FLOW24] Firing kb_Add(kbeUPDATE) for %s @ %s', [rls, ps.Name]));
...
begin
  ...
  try
    ...
    if ImdbDatabase <> nil then
      ...
      Debug(dpError, section, 'IMDb database could not be opened, continuing without persistent IMDB data');
  except
    on e: Exception do
...
begin
  ...
  begin
    getShowValues(rls, fShowname, fSeason, fEpisode);
    (*
      if getShowValues does not find tv-related info for rls then fShowname will
      contain the same value as rls, otherwise it will contain a (shorter)
      showname. fSeason and/or fEpisode will be set if the release is tv-related.
    *)
    if (rls <> fShowname) and ((fSeason > 0) or (fEpisode > 0)) then
      exit;
  end;
            ...
            try
              ...
              begin
                ...
                begin
                  ...
                  if (ps <> nil) then
                  begin
                    Debug(dpSpam, section, Format('[IMDB-FLOW8h] Firing kb_Add(kbeUPDATE) for %s @ %s', [fRls, ps.Name]));
...
begin
  try
    ...
    begin
      Debug(dpSpam, section, Format('[IMDB-FLOW11] Release %s already in database, skipping task creation', [aReleaseName]));
    ...
    begin
      Debug(dpSpam, section, Format('[IMDB-FLOW12] IMDB-ID %s already in database, skipping task creation', [aIMDbId]));
      Exit;
    end;
    ...
    try
      ...
      begin
        Debug(dpSpam, section, Format('[IMDB-FLOW13] Task already pending for %s, skipping duplicate', [aReleaseName]));
        Exit;
      end;
      if running_imdb_tasks.IndexOf(aReleaseName) <> -1 then
      begin
        Debug(dpSpam, section, Format('[IMDB-FLOW14] Task already running for %s, skipping duplicate', [aReleaseName]));
        Exit;
      end;
    ...
    Debug(dpSpam, section, Format('[IMDB-FLOW15] Creating IMDB API task for %s, IMDB-ID: %s', [aReleaseName, aIMDbId]));
    AddTask(TPazoHTTPImdbTask.Create(aIMDbId, aReleaseName), true);
    Debug(dpSpam, section, Format('[IMDB-FLOW16] Task object created and added to task queue for %s', [aReleaseName]));
  except
    on e: Exception do
    begin
