...
set CC_ND_EXTRAS=-NSWinapi;System.Win;Data.Win;Datasnap.Win;Web.Win;Soap.Win;Xml.Win;Bde;System;Xml;Data;Datasnap;Web;Soap;DUnitX.MemoryLeakMonitor
set CFLAGS=-B -$O+,C+,D-,L-
set CDBFLAGS=-B -$O+,C+,D+,L+
set CINCLUDES=-Ulibs/FastMM4 -Ulibs/BeRoHighResolutionTimer -Ulibs/FLRE -Ulibs/rcmdline -Ulibs/DFFLibV15_UIntList -Ulibs/lkJSON -Ulibs/TRegExpr -Ulibs/pasmp -Ulibs/Compvers -Ulibs/ctypes -Ulibs/Indy10_5448/Core -Ulibs/Indy10_5448/Protocols -Ulibs/Indy10_5448/System -Ulibs/LibTar
...
echo "%CC%" %CFLAGS% %CINCLUDES% slftp.dpr
if errorlevel 1 (
...
echo "%CC%" %CDBFLAGS% %CINCLUDES% slftp.dpr
...
echo "%CC_ND_32%" %CFLAGS% %CC_ND_EXTRAS% %CINCLUDES% slftp.dpr
"%CC_ND_32%" %CFLAGS% %CC_ND_EXTRAS% %CINCLUDES% slftp.dpr
if errorlevel 1 (
   echo Failure Reason Given is %errorlevel%
   exit /b %errorlevel%
)
...
"%CC_ND_64%" %CFLAGS% %CC_ND_EXTRAS% %CINCLUDES% slftp.dpr
if errorlevel 1 (
   echo Failure Reason Given is %errorlevel%
   exit /b %errorlevel%
)
...
"%CC_ND_32%" %CDBFLAGS% %CC_ND_EXTRAS% %CINCLUDES% slftp.dpr
if errorlevel 1 (
   echo Failure Reason Given is %errorlevel%
   exit /b %errorlevel%
)
...
"%CC_ND_64%" %CDBFLAGS% %CC_ND_EXTRAS% %CINCLUDES% slftp.dpr
if errorlevel 1 (
   echo Failure Reason Given is %errorlevel%
   exit /b %errorlevel%
)
